...
// --- Presentational Component: Task Card (with Drag Handle) ---
function TaskCard({ task, onCardClick }) {
    const { attributes, listeners, setNodeRef, transform } = useDraggable({ id: task.id });
    const style = transform ? { transform: `translate3d(${transform.x}px, ${transform.y}px, 0)` } : undefined;
    return (
        <div ref={setNodeRef} style={style} {...attributes} onClick={onCardClick} className="bg-slate-700 p-3 rounded-md shadow-md mb-3 group relative touch-none">
            {/* The drag handle is a dedicated element. The listeners are ONLY here. */}
            <button {...listeners} className="absolute top-1/2 -translate-y-1/2 left-0 w-2 h-8 bg-slate-600/50 rounded-r-full cursor-grab opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Drag task"></button>
...
function KanbanTaskBoard() {
    ...
    const calculateProgress = useCallback((currentColumns) => { const doneTasks = currentColumns.done?.tasks.length || 0; const totalTasks = Object.values(currentColumns).reduce((acc, col) => acc + col.tasks.length, 0); if (totalTasks === 0) return 100; return (doneTasks / totalTasks) * 100; }, []);
    const saveBoard = useCallback(async (boardData) => { if (!user) return false; const { error: saveError } = await supabase.from('boards').upsert(boardData, { onConflict: 'id, user_id' }); if (saveError) { setError(saveError.message); return false; } setError(null); return true; }, [date, user]);
    useEffect(() => { const fetchBoardForDate = async () => { if (!date || !user) return; setIsLoading(true); setError(null); const { data, error: fetchError } = await supabase.from('boards').select('*').eq('id', date).eq('user_id', user.id).single(); if (fetchError && fetchError.code !== 'PGRST116') { setError(fetchError.message); } else if (data) { const columnsObject = data.columns.reduce((acc, col) => { acc[col.id] = col; return acc; }, {}); setColumns(columnsObject); setProgress(calculateProgress(columnsObject)); } else { const newBoard = createNewBoardData(date, user.id); const success = await saveBoard(newBoard); if (success) { const columnsObject = newBoard.columns.reduce((acc, col) => { acc[col.id] = col; return acc; }, {}); setColumns(columnsObject); setProgress(calculateProgress(columnsObject)); } } setIsLoading(false); }; fetchBoardForDate(); }, [date, user, saveBoard, calculateProgress]);
    ...
    const handleAddTask = async (event) => { event.preventDefault(); const { title, priority, assigneeName, column, projectName } = event.target.elements; const newTask = { id: `task-${Date.now()}`, title: title.value, priority: priority.value, assigneeName: assigneeName.value, projectName: projectName.value }; const targetColumnId = column.value; const updatedColumns = { ...columns, [targetColumnId]: { ...columns[targetColumnId], tasks: [...columns[targetColumnId].tasks, newTask] } }; const success = await saveBoard(createSaveObject(updatedColumns)); if (success) { setColumns(updatedColumns); setProgress(calculateProgress(updatedColumns)); setIsAddModalOpen(false); } };
    const handleDeleteTask = async () => { if (!taskToEdit) return; const { task, columnId } = taskToEdit; const updatedColumns = { ...columns }; updatedColumns[columnId].tasks = updatedColumns[columnId].tasks.filter(t => t.id !== task.id); const success = await saveBoard(createSaveObject(updatedColumns)); if (success) { setColumns(updatedColumns); setProgress(calculateProgress(updatedColumns)); setTaskToEdit(null); setTaskToDelete(null); } };
    const handleEditTask = async (event) => { event.preventDefault(); if (!taskToEdit) return; const { title, priority, assigneeName, projectName } = event.target.elements; const { task, columnId } = taskToEdit; const updatedTask = { ...task, title: title.value, priority: priority.value, assigneeName: assigneeName.value, projectName: projectName.value }; const updatedColumns = { ...columns }; const taskIndex = updatedColumns[columnId].tasks.findIndex(t => t.id === task.id); if (taskIndex !== -1) { updatedColumns[columnId].tasks[taskIndex] = updatedTask; } const success = await saveBoard(createSaveObject(updatedColumns)); if (success) { setColumns(updatedColumns); setTaskToEdit(null); } };

    if (isLoading) return <div className="min-h-screen bg-slate-900 text-white flex items-center justify-center text-2xl">Loading Board...</div>;
    if (error) return <div className="min-h-screen bg-slate-900 text-red-500 flex items-center justify-center text-2xl text-center p-4">Error: {error}<br /><button onClick={() => window.location.reload()} className="text-sm mt-4 p-2 bg-slate-700 rounded-md">Reload</button></div>;
    ...
    return (
        <DndContext onDragEnd={handleDragEnd}>
            <div className="min-h-screen w-full bg-slate-900 flex flex-col p-4 md:p-8">
                <div className="mb-6"><Link to="/calendar" className="text-blue-400 hover:text-blue-300 transition-colors mb-2 block">← Back to Calendar</Link><div className="flex justify-between items-center"><h1 className="text-3xl font-bold text-white">Your tasks for {formattedDate}</h1><button onClick={() => setIsAddModalOpen(true)} className="bg-green-500 text-white font-bold px-4 py-2 rounded-lg hover:bg-green-600 transition-colors">+ Add Task</button></div><ProgressMeter percentage={progress} /></div>
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">{Object.values(columns).length > 0 ? (Object.values(columns).map(column => (<Column key={column.id} column={column} openEditModal={(task, colId) => setTaskToEdit({ task, columnId: colId })} />))) : (<p className="text-slate-400 col-span-4">No columns found for this board.</p>)}</div>
            </div>
